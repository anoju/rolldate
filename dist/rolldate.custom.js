...
(function (global, factory) {
	...
	Rolldate.prototype = {
		...
		createUI: function createUI() {
			...
			var rangeArry = config.yearRange.split(':'),
				rangeS = parseInt(rangeArry[0]),
				rangeE = parseInt(rangeArry[1]);
			if(config.minDate == null){
				config.minDate = (parseInt($rdTodayY)-rangeS)+$rdTodayMD;
			}else if(config.minDate == 'today'){
				config.minDate = $rdToday;
			}
			if(config.maxDate == null){
				config.maxDate = (parseInt($rdTodayY)+rangeE)+$rdTodayMD;
			}else if(config.maxDate == 'today'){
				config.maxDate = $rdToday;
			}
			var beginYear = parseInt(config.minDate.substr(0,4)),
				isBeginYear = false,
				endYear = parseInt(config.maxDate.substr(0,4)),
				isEndYear = false,
				beginMonth = parseInt(config.minDate.substr(4,2)),
				isBeginMonth = false,
				endMonth = parseInt(config.maxDate.substr(4,2)),
				isEndMonth = false,
				beginDay = parseInt(config.minDate.substr(6,2)),
				endDay = parseInt(config.maxDate.substr(6,2));
			...
			for (var i = 0; i < len; i++) {
				...
				//console.log(config.minDate,config.maxDate)
				
				if (f == 'YYYY') {
					for (var j = beginYear; j <= endYear; j++) {
						//isBeginYear = beginYear == parseInt(date.getFullYear()) ? true : false;
						//isEndYear = endYear == parseInt(date.getFullYear()) ? true : false;
						if(beginYear == parseInt(date.getFullYear()))isBeginYear = true;
						if(endYear == parseInt(date.getFullYear()))isEndYear = true;
					...
					var k_s = 1,
						k_e = 12;
					if(isBeginYear)k_s = beginMonth;
					if(isEndYear)k_e = endMonth;
					if(beginMonth == parseInt(date.getMonth()) + 1)isBeginMonth = true;
					if(endMonth == parseInt(date.getMonth()) + 1)isEndMonth = true;
					for (var k = k_s; k <= k_e; k++) {
					...
					var day = _this.getMonthlyDay(date.getFullYear(), date.getMonth() + 1),
						l_s = 1,
						l_e = day;
					if(isBeginYear && isBeginMonth)l_s = beginDay;
					if(isEndYear && isEndMonth)l_e = endDay;
					for (var l = l_s; l <= l_e; l++) {
			...
			var $html = '<div class="rolldate-mask"></div><div class="rolldate-panel"><header><button type="button" class="rolldate-btn rolldate-cancel">' + lang.cancel + '</button>' + lang.title + '<button type="button" class="rolldate-btn rolldate-confirm">' + lang.confirm + '</button></header><section class="rolldate-content"><div class="rolldate-dim mask-top"></div><div class="rolldate-dim mask-bottom"></div><div class="rolldate-wrapper">' + ul + '</div></section></div>',
			...
			var _loop = function _loop(_i) {
				...
				that.wheelTo(index);
				// 滚动结束
				that.on('bscrollEnd', function (){
					if (config.moveEnd) {
						config.moveEnd.call(_this, that);
					}

					var _selectedYear = parseInt(_this.getSelected(_this.scroll['YYYY'])),
						_selectedMonth = parseInt(_this.getSelected(_this.scroll['MM']));
					//월 새로그림
					if ([domId['YYYY']].indexOf(that.wrapper.id) != -1 && _this.scroll['YYYY'] && _this.scroll['MM'] && _this.scroll['DD']) {
						var _monthLi = '',
							_monthFirst = 1,
							_monthLast = 12;
							
						if(_selectedYear == beginYear)_monthFirst = beginMonth;
						if(_selectedYear == endYear)_monthLast = endMonth;
						if ((_monthLast-_monthFirst+1) != $('#' + domId['MM'] + ' li', 1).length) {
							//console.log('month refresh');
							for (var _monthI = _monthFirst; _monthI <= _monthLast; _monthI++) {
								_monthLi += '<li class="wheel-item">' + (_monthI < 10 ? '0' + _monthI : _monthI) + lang.month + '</li>';
							}
							$('#' + domId['MM'] + ' ul').innerHTML = _monthLi;
							_this.scroll['MM'].refresh();
							if(_selectedYear == beginYear)_this.scroll['MM'].wheelTo(0);
						}
					}
					...
					if ([domId['YYYY'], domId['MM']].indexOf(that.wrapper.id) != -1 && _this.scroll['YYYY'] && _this.scroll['MM'] && _this.scroll['DD']) {
						var _day = _this.getMonthlyDay(_this.getSelected(_this.scroll['YYYY']), _this.getSelected(_this.scroll['MM'])),
							...
							_dayFirst = 1,
							_dayLast = _day;
						if(_selectedYear == beginYear && _selectedMonth < beginMonth)_selectedMonth = beginMonth;
						...
						if ((_dayLast-_dayFirst+1) != $('#' + domId['DD'] + ' li', 1).length) {
							...
							$('#' + domId['DD'] + ' ul').innerHTML = _dayLi;
							_this.scroll['DD'].refresh();
							if(_selectedYear == beginYear && _selectedMonth == beginMonth)_this.scroll['DD'].wheelTo(0);
						}
					}
				});
			};
...
//날짜구하기
var rdTodayTimeString=function(addDay){
	var $today=new Date();
	if(!!addDay)$today.setDate($today.getDate()+addDay);
	return rdTimeString($today);
};
var rdTimeString=function(date){
...
var $rdToday=rdTodayTimeString(),					//년+월+일
	$rdTodayYM=rdTodayTimeString().substr(0,6),		//년+월
	$rdTodayMD=rdTodayTimeString().substr(4,4),		//월+일
	$rdTodayY=rdTodayTimeString().substr(0,4),		//년
	$rdTodayM=rdTodayTimeString().substr(4,2),		//월
	$rdTodayD=rdTodayTimeString().substr(6,2);		//일